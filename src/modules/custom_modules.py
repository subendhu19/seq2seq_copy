...
from allennlp.modules import Attention, TextFieldEmbedder
...
@DatasetReader.register("copynet_seq2seq_bert")
class CopyNetDatasetReader(DatasetReader):
    ...
    def text_to_instance(
        ...
        meta_fields = {"source_tokens": [x.text for x in tokenized_source[1:-1]],
                       "bert_tokens": [x.text for x in source_field]}
...
@Model.register("copynet_seq2seq_bert")
class CopyNetSeq2Seq(Model):
    ...
    def __init__(
        ...
        # Encoding modules.
        bert_token_embedding = PretrainedBertEmbedder('bert-base-uncased', requires_grad=True)

        self._source_embedder = bert_token_embedding
        self._encoder = PassThroughEncoder(input_dim=self._source_embedder.get_output_dim())
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        embedded_input = self._source_embedder.forward(source_tokens['bert'], source_tokens['bert-offsets'])
